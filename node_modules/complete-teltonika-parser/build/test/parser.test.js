...
const ProtocolParser_1 = require("../Scripts/ProtocolParser");
test('Packet 1', () => {
    ...
    expect(() => new ProtocolParser_1.ProtocolParser(packet)).not.toThrow();
    let { Quantity1, Content, CodecType } = new ProtocolParser_1.ProtocolParser(packet);
    expect(Quantity1).toBe(1);
    expect(CodecType).toEqual('data sending');
    let { AVL_Datas } = Content;
    expect(AVL_Datas.length).toEqual(1);
    let [AVL_Data] = AVL_Datas;
    let { GPSelement: { Latitude, Longitude, Altitude, Angle, Satellites, Speed }, IOelement: { Elements, ElementCount }, Timestamp } = AVL_Data;
